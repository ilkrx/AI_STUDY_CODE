...
# 定义心知天气API的工具类
class WeatherTool:
    city: str = Field(description="City name, include city and county,北京/长沙/武汉")

    def __init__(self, api_key) -> None:
        self.api_key = api_key

    def run(self, city):
        city = city.split("\n")[0].strip()  # 清除多余的换行符，避免报错
        ...
        if response.status_code == 200:  # 请求成功
            ...
            error_msg = f"无法获取{city}的天气信息。状态码: {response.status_code}, 响应内容: {response.text}"
